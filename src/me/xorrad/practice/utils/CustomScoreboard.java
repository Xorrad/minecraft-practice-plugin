...
public class CustomScoreboard
{
     ...
     *
     */
    private static Class<?> OBJECTIVE_PACKET_CLASS = Reflection.getClass("{nms}.PacketPlayOutScoreboardObjective");
    private static Field O_NAME = Reflection.getField(OBJECTIVE_PACKET_CLASS, "a");
    private static Field O_VALUE = Reflection.getField(OBJECTIVE_PACKET_CLASS, "b");
    private static Field O_MODE = Reflection.getField(OBJECTIVE_PACKET_CLASS, "c");

    private static Class<?> DISPLAY_OBJECTIVE_PACKET_CLASS = Reflection.getClass("{nms}.PacketPlayOutScoreboardDisplayObjective");
    private static Field DO_SLOT = Reflection.getField(DISPLAY_OBJECTIVE_PACKET_CLASS, "a");
    private static Field DO_OBJ_NAME = Reflection.getField(DISPLAY_OBJECTIVE_PACKET_CLASS, "b");

    private static Class<?> SCORE_PACKET_CLASS = Reflection.getClass("{nms}.PacketPlayOutScoreboardScore");
    @SuppressWarnings("unused")
	private static Field S_SCORE_NAME = Reflection.getField(SCORE_PACKET_CLASS, "a");
    private static Field S_OBJ_NAME = Reflection.getField(SCORE_PACKET_CLASS, "b");
    private static Field S_SCORE_INT = Reflection.getField(SCORE_PACKET_CLASS, "c");
    private static Field S_ACTION = Reflection.getField(SCORE_PACKET_CLASS, "d");

    private Team[] lines = new Team[15];
     ...
     */
    public CustomScoreboard(UUID playerUUID, String objectiveName, ScoreboardSlot slot)
    {
    ...
    /**
     * Send the packets to update this scoreboard. A scoreboard must be created using {@link CustomScoreboard#create()} in order
     * to be used
     */
    public void update()
    {
    ...
    /**
     * Send the packets to remove this scoreboard. A destroyed scoreboard sign must be recreated using {@link CustomScoreboard#create()} in order
     * to be used again
     */
    public void destroy()
    {
    ...
    private Object sendScore(String line, int score)
    {
        try
        {
            ...
            Reflection.setFieldValue(packet, S_ACTION, 0);
            return packet;
        } catch (Exception e)
        {
            e.printStackTrace();
